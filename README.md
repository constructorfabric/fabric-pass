...
| `CONTRIBUTORS_SEED_SECRET` | **Optional** — shared secret guarding `POST /internal/contributors/seed-from-org` (see [Contributors registry sync](#contributors-registry-sync)). With it unset, that route answers `503` and is effectively disabled — an install that never seeds from org membership doesn't need this secret at all. Its own secret, not a reuse of `CONTRIBUTORS_SYNC_SECRET`, so either can be rotated or revoked independently even though both guard routes under the same `/internal/contributors/*` path |
...
The rows above `RESEND_*`, except `CONTRIBUTORS_SEED_SECRET`, are required, not just for running the app: `src/lib/env.ts` validates the whole environment at import, and `next build` imports every route module while collecting page data, so `pnpm build` fails before it reaches any provider if even one variable is unset. Placeholder values satisfy this — the build never contacts a provider. `RESEND_*`, `LINKEDIN_CLIENT_ID`/`LINKEDIN_CLIENT_SECRET`, `ROOT_GITHUB_ID`, `DO_API_TOKEN`/`DO_DROPLET_ID`, `GITHUB_ORG_TOKEN`/`DISCORD_BOT_TOKEN`, and `CONTRIBUTORS_SEED_SECRET` are the exceptions, deliberately: this app needs to keep booting in an environment where email, LinkedIn, a root user, droplet monitoring, the auto-invite feature, or GitHub-org contributor seeding hasn't been configured yet.
...
### Seeding contributors from GitHub org membership

IDEA-143 — the sync above can only ever *update* a row already matched by `github_id`, so an org member who has never signed in through GitHub OAuth has no row for it to match at all. `POST /internal/contributors/seed-from-org` (bearer-protected by `CONTRIBUTORS_SEED_SECRET`) closes that gap on demand: it creates a `draft` contributors row for every member of `github_organization` (see [App config](#app-config)) this app doesn't already have one for, carrying whatever GitHub's *public* profile offers — a name and email, if the account holder has made either public. It only ever creates rows; an existing contributor is never touched, even if their public GitHub profile now shows a different name or email. A seeded row lands with `status: draft` and `profile_completeness: incomplete`, same as any other freshly created row, so it's invisible to contributor search until the person signs in themselves and an admin confirms them the normal way. Needs `GITHUB_ORG_TOKEN` (see [Auto-invite and track access](#auto-invite-and-track-access)) — the request fails with `503` if it's unset, since the whole point of this route is the GitHub read.

`CONTRIBUTORS_SEED_SECRET` is optional, unlike the sync secrets above: this route is an on-demand maintenance action, not part of a sync any deployment depends on. With it unset, the route answers `503` and a deployment simply has no seeding route at all — deliberately, so that adding this feature never forces an existing deployment to hold a new secret before it can take the update.
...
The `tracks` table (`migrations/010_tracks.sql`) and its `track_admins` join table are entirely owned by `pass/tracks.yaml` in cf-internal, synced one-way (file → DB only, `POST /internal/tracks/sync`, bearer-protected by `TRACKS_SYNC_SECRET`) — unlike contributors, nothing about a track is self-reported by any one person, so there's nothing here for this app to export back. Each track has a `slug` (its stable key), `name`, `description`, a `repositories` list (URL, description, issue-tracker link — stored as `jsonb`, always read/written as one small unit), and up to six named leader slots (Product Manager, Architect, Developer, Quality, Researcher, and Governance — the last a non-technical, administrative role any track can use, not just Governance's own). Leaders are written in the file as GitHub *logins* (`src/lib/tracks-registry.ts`), not the numeric `github_id`s the rest of this app keys on — a login is what a human hand-editing the file actually knows and can eyeball-verify. `src/lib/tracks.ts`'s `syncTracks` resolves each login to its contributor's `github_id` at sync time; the database itself still stores the id, the stable key, since a login can be renamed.
