...
Part of [Constructor Fabric](https://constructorfabric.org). Fabric Pass is a directory of an open-source project's contributors, collected through a single link. A contributor signs in with GitHub, which creates their one row immediately, keyed by GitHub account; from there, filling in a short profile and linking Telegram and/or Discord each autosave as they happen, with no separate save step for any one field. Returning and signing in again reopens the existing row, read-only until Edit is clicked — unless Name or Email is still missing, in which case sign-in opens it straight into edit mode instead.
