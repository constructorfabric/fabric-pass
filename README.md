...
| `CONTRIBUTORS_SEED_SECRET` | Shared secret guarding `POST /internal/contributors/seed-from-org` (see [Contributors registry sync](#contributors-registry-sync)) — its own secret, not a reuse of `CONTRIBUTORS_SYNC_SECRET`, so either can be rotated or revoked independently even though both guard routes under the same `/internal/contributors/*` path |
